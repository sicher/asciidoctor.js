...
w)}((d=!1!==(p=D(w,u))&&p!==c?p:(n=!1!==m&&m!==c)?D(w,m):m)===c||d.$$is_boolean&&1!=d)&&A.$raise(a.get("ArgumentError").$new("Invalid type for "+k+" extension: "+w));z=w.$new(y);z.$freeze();return f.get("ProcessorExtension").$new(b,z)}();return t.$config()["$[]"]("position")["$=="](">>")?v.$unshift(t):v["$<<"](t)});a.defn(b,"$add_syntax_processor",G=function(b,e){var d,g,r,h,p;p=G.$$p||c;var n=c,k=c,q=c,t=k=c,v=q=c,y=c,m=c,k=c;G.$$p=null;n=b.$to_s().$tr("_"," ");k=(d=(g=n.$split(" ")).$map,d.$$p=
...
a.get("Object");(d=(e=b.$split("::")).$each,d.$$p=(f=function(a){var d=f.$$s||this,e;null==a&&(a=c);return(e=a["$empty?"]())===c||e.$$is_boolean&&1!=e?(e=k["$const_defined?"](a))===c||e.$$is_boolean&&1!=e?d.$raise("Could not resolve class for name: "+b):k=k.$const_get(a):c},f.$$s=this,f),d).call(e);return k}),c)&&"class_for_name"})(a.get_singleton_class(z))})(t(K,"Asciidoctor").$$scope.base)}(a.base)})(B);(function(a){a.dynamic_require_severity="ignore";return a.nil})(B)};
